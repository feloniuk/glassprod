<?php
// Файл: dashboard/grain_requests.php
// Управління заявками на закупку зернової сировини
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        $purpose = $_POST['purpose'] ?? 'production';
        ...
        if ($materialId && $quantity > 0) {
            ...
            $requestId = $grainPurchaseRequest->createRequest($requestData);
            if ($requestId) {
    ...
    elseif ($action === 'approve' && in_array($user['role'], ['director', 'procurement_manager'])) {
        ...
        if ($requestId && $grainPurchaseRequest->approve($requestId, $user['id'])) {
    ...
    elseif ($action === 'reject' && in_array($user['role'], ['director', 'procurement_manager'])) {
        ...
        if ($requestId && $grainPurchaseRequest->reject($requestId, $user['id'], $comments)) {
    ...
    elseif ($action === 'bulk_create' && $user['role'] === 'warehouse_keeper') {
        ...
        foreach ($lowStockMaterials as $mat) {
            ...
            if ($grainPurchaseRequest->createRequest($requestData)) {
                $created++;
            }
        }
...
if ($user['role'] === 'warehouse_keeper') {
    ...
    $requests = $grainPurchaseRequest->getByUser($user['id']);
} else {
    // Директор і менеджер бачать всі заявки
    $requests = $grainPurchaseRequest->getAllWithDetails();
}
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form method="GET" class="row g-3">
                    ...
                    <div class="col-md-3">
                        <label class="form-label">&nbsp;</label>
                        <a href="<?= BASE_URL ?>/dashboard/grain_requests.php" class="btn btn-outline-secondary d-block">
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">
            ...
            Заявки на закупку зернової сировини (<?= count($requests) ?>)
        </h5>
    </div>
            ...
            <form method="POST">
                <div class="modal-header">
                    <h5 class="modal-title">
                        <i class="fas fa-plus me-2"></i>Нова заявка на зерно
                    </h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="purpose" class="form-select">
                            ...
                            <option value="quality_improvement">Підвищення якості</option>
                        </select>
                    </div>
                    ...
                    <div class="alert alert-info" id="alcoholYieldInfo" style="display: none;">
                        <strong>Потенційний вихід спирту:</strong> <span id="alcoholYield">0</span> літрів
                    </div>
...
$additionalJS = "
// Розрахунок вартості та виходу спирту при зміні кількості або матеріалу
document.addEventListener('DOMContentLoaded', function() {
    ...
    const alcoholYieldInfo = document.getElementById('alcoholYieldInfo');
    const alcoholYieldDisplay = document.getElementById('alcoholYield');
    ...
    function updateCalculations() {
        ...
        if (selectedOption && quantityInput.value) {
            ...
            const alcoholYield = parseFloat(selectedOption.dataset.yield || 0);
            ...
            if (alcoholYield > 0 && quantity > 0) {
                const totalYield = alcoholYield * quantity;
                alcoholYieldDisplay.textContent = totalYield.toLocaleString('uk-UA');
                alcoholYieldInfo.style.display = 'block';
            } else {
                alcoholYieldInfo.style.display = 'none';
            }
        } else {
            costEstimate.style.display = 'none';
            alcoholYieldInfo.style.display = 'none';
...
function showRequestDetails(request) {
    ...
    const purposeText = {
        ...
        'reserve': 'Резерв',
        'quality_improvement': 'Підвищення якості'
    };
        ...
        <div class='row'>
            <div class='col-md-6'>
                ...
                <table class='table table-sm'>
                    ...
                    <tr><th>Зернова сировина:</th><td>\${request.material_name}</td></tr>
                    ...
                    <tr><th>Пріоритет:</th><td><span class='badge \${getPriorityBadgeClass(request.priority)}'>\${getPriorityText(request.priority)}</span></td></tr>
                    <tr><th>Призначення:</th><td>\${purposeText[request.purpose] || request.purpose}</td></tr>
                </table>
            </div>
            <div class='col-md-6'>
                <h6>Статус та дати</h6>
                <table class='table table-sm'>
                    <tr><th>Статус:</th><td><span class='badge \${getStatusBadgeClass(request.status)}'>\${getStatusText(request.status)}</span></td></tr>
...
renderDashboardLayout('Заявки на закупку зерна', $user['role'], $content, '', $additionalJS);
?>